...
import { OptionProps, User, UserRequest } from "@/types";
...
const AccountForm = ({
  ...
  const form = useForm<AccountFormProps>({
    initialValues: {
      ...
      rePassword: "",
      roles:
        user?.role || roles(currentUser?.role as string)[0].id || "",
    },
    validate: {
  ...
  const onSelectChain = (chain: string | null) => {
    setChainId(chain);
  };
  ...
  const displayChains = useMemo(() => {
    return (
      !["ADMIN"].includes(form.values.roles) && chains.length > 0
    );
  }, [chains.length, form.values.roles]);
  const displayBranches = useMemo(() => {
    return (
      !["ADMIN", "OWNER", "CHAIN-MANAGER"].includes(
        form.values.roles,
      ) && branches.length > 0
    );
  }, [branches.length, form.values.roles]);
  ...
  const onSubmit = useCallback(
    ...
    [branchId, chainId, error, form, onSave],
  );
  useWatchProp(error, () => {
    !error && form.reset();
  });
  ...
  const OPTION_CHAIN: OptionProps[] = useMemo(() => {
    return chains.map(({ id, name }) => ({ id, name }));
  }, [chains]);
        ...
        <Stack gap={20} px={10} my={20}>
          ...
          <div className={classes.radioContainer}>
            ...
            <RadioGroup
              ...
              options={roles(currentUser?.role as string)}
