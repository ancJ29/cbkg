...
const shouldParseParams = import.meta.env.DEV;
function _validateParams<T>(action: string, params?: T) {
  if (!shouldParseParams) {
    return params;
  }
  let _params: T | undefined = params;
  const actionConfig = config[
    action as keyof typeof config
  ] as ActionConfig;
  if (actionConfig?.schema?.request) {
    const res = actionConfig.schema.request.safeParse(params);
    if (!res.success) {
      logger.error("[api-v2-invalid-params]", res.error);
    } else {
      _params = res.data as T;
    }
  }
  return _params;
}

export default async function callApiV3<T, R>({
  params,
  action,
  options = {},
}: callApiProps<T>) {
