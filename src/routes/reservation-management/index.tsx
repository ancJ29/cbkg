      ...
      <Card withBorder shadow="md" w="100%">
        ...
        <Flex my={20} justify="space-between" align="flex-start">
          {isMobile ? (
            ...
            >
              {/* TODO: filter */}
              <FilterReservationForm onFilter={() => closeDrawer()} />
            </Drawer>
          ) : (
...
const _records = (data?: Reservation) => {
  return [
    ...
    },
    {
      style: { flex: 2 },
      label: "From",
      value: <Text>{data?.from?.format("YYYY-MM-DD") || "-"}</Text>,
    },
    {
      style: { flex: 2 },
      label: "To",
      value: <Text>{data?.to?.format("YYYY-MM-DD") || "-"}</Text>,
    },
    {
      style: { flex: 2 },
      label: "Table",
